...
istruzione richiama i compilatori di riferimento, così da gestire nel miglior
...
minor numero di revision number. Se devices diversi hanno il più
alto revision number uguale sta a significare che posseggono la stessa
architettura. Il più alto numero di revision number per le architetture Kepler è
3, per i devices basati su un'architettura Fermi è 2, mentre per i device con
architettura Tesla 1. Il numero minore di revision number invece, corrisponde ad
un miglioramento incrementale dell'architettura di base che spesso può
comportare nuove funzionalità.
...
chiamava ``GK104" in cui ogni unità  interna fu progettata ai fini di
...
particolare è il ``GK107" che offre due diversi blocchi di shader cores,
...
matrici può essere implementata in un kernel dove ogni threads computa un
elemento dell'output. Il massimo del parallelismo si ha quando ad ogni threads è
...
questo punto di vista, dunque il programmatore può tralasciare questo tipo di
problema.
...
utilizzare il parallelismo dinamico \ref{par:Parallelismo_dinamico}), genererà
una griglia di threads sul device. CUDA genera threads suddivisi in blocchi, ed
ogni blocco appartiene ad una griglia. Lo schema è mostrato in figura
\ref{fig:grid_block}.
...
blocco. L'identificazione di un thread è strettamente necessario nel calcolo
parallelo, per questo c'è bisogno di un meccanismo per distinguere diversi
...
\texttt{x} avrà valore 3, la \texttt{y} 2 e la \texttt{z} 2. Nel caso in cui
invece dichiarassimo \texttt{dim3 dimGrid(3)} il compilatore comprende che
vogliamo solamente utilizzare una dimensione e imposterà la \texttt{y} e la
\texttt{z} ad 1 automaticamente.

Non dimentichiamo però che le dimensioni di griglie e blocchi vengono definite
...
Come si può intuire una funzione di tipo \texttt{``\_\_device\_\_''} può essere
richiamata dai kernels e dunque verrà lanciata lato device, mentre
\texttt{``\_\_host\_\_''} sarà una funzione che verrà richiamata lato host, in
...
scrivere e modificare dati presenti sulla shared memory ma non può eseguire
...
Una motivazione per cui utilizzare la memoria condivisa è la differenza di
velocità rispetto alla global memory. Già con semplici esempi come la
moltiplicazione tra matrici, si può notare come l'utilizzo della shared memory
rispetto alla global memory, comporta un miglioramento di performance.
Un altra differenza rispetto alla global memory è che al termine delle
operazioni del kernel la shared memory terminerà il suo lavoro rilasciando i
dati salvati in precedenza mentre la global memory mantiene le informazioni
fino alla fine di tutto il programma.

La shared memory è suddivisa in banks, in cui ogni bank può
...
sulla memoria costante è: \texttt{``\_\_constant\_\_''}.
...
Le più note operazioni implementate dalle API di CUDA sono quelle relative alle
operazioni aritmetiche.
Facciamo un breve elenco delle funzioni più conosciute:
...
Creare un kernel direttamente da GPU può ridurre il bisogno di trasferire dati
tra host e device così come riduce il controllo dell'esecuzione e
...
direttamente dal device. La stessa opportunità si ha per il parallelismo dei
dati che può essere generato direttamente all'interno di un kernel, così da
...
implementare un programma CUDA. La distinzione fondamentale tra i due è il
...
Un altro tool degno di menzione è \textbf{CUDA-MEMCHECK} \cite{CTD}, incluso in
