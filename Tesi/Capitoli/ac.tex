...
Oggigiorno, dopo decadi di sviluppo e ricerca, la scienza ricopre un ruolo
...
(AC) sono modelli matematici usati per la simulazione di sistemi complessi. Un AC è
sostanzialmente uno spazio d-dimensionale suddiviso in celle regolari alle quali
è associato uno \textbf{stato} facente parte di un insieme finito di stati. Il
valore dello stato di una cella in un determinato istante è dato da una
\textbf{funzione di transizione} uguale per tutte le celle. La funzione di
transizione calcola lo stato di una cella in base allo stato del suo
\textbf{vicinato}, ossia un insieme di celle legate da una prefissata relazione.
Tutto ciò avviene in passi discreti e lo stato di tutte le celle è aggiornato in
modo simultaneo. L'insieme degli stati di tutte le celle in un istante $t$
determina la configurazione dell' Automa Cellulare in quell' istante.
\begin{figure}
\centering
\subfloat[][]
