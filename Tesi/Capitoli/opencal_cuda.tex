...
\textsf{CudaCALModel2D}, tra le diverse implementazioni si
...
(\textsf{device\_ca2D}). La presenza di \textsf{h\_device\_ca2D} invece è
...
\textsf{device\_ca2D}. L'utente ha il compito di dichiarare un istanza del
...
funzione \textsf{calCudaRunDef2D}.
...
particolare il metodo \textsf{calCudaAllocatorModel} (cod. \ref{lst:copyhd})prende in input il modello
...
la più importante, è sicuramente la matrice di \textsf{FLAGS}. Quest'ultima
possiamo rappresentarla come una matrice booleana in cui il valore
\textsf{TRUE} nella posizione
...
\textsf{CALSubstate2D(r|i|b)}, come spiegato nei paragrafi precedenti tutto ciò
